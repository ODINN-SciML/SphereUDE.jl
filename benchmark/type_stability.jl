...
println("Assessing type stability")
...
# @code_warntype train(data, params, rng, nothing, nothing)

U = get_default_NN(params, rng, nothing)
_θ, st = SphereUDE.Lux.setup(rng, U)
θ = SphereUDE.convert_to_float64(_θ) # Convert to Float64 to avoid type instabilities
β = SphereUDE.ComponentVector{Float64}(θ = θ, u0 = params.u0)
# Closure of the ODE update for solve
function ude_rotation_closure!(du, u, p, t)
    SphereUDE.ude_rotation!(du, u, p, t, U, st)
end

prob_nn = ODEProblem(ude_rotation_closure!, params.u0, [params.tmin, params.tmax], β.θ)

@code_warntype SphereUDE.predict(β, prob_nn, params, data.times)

# @code_warntype SphereUDE.loss_empirical(β, prob_nn, data, params)

# @code_warntype loss(β, prob_nn, data, params, U, st)